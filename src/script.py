def main():
    # get nation name from tkinter
    headers = {
        "user-agent": f"Hellfire v. 1.0.0 by Celinova, in use by {user_agent}",
    ...
    try:
        ...
        print(
            f"{now()} Authentication successful!"
            if authenticated
            else f"{now()} Authentication failed."
        )
        ...
        while True:
            ...
            reports_response = session._html_request(
                reports_url, auth=(nation_name, password), data=report_settings
            )

            current_time = time.time()  # get the current time
            elapsed_time = (
                (current_time - last_time) * 1000
            ).__round__()  # calculate the elapsed time in milliseconds
            ...
            for report in reports_soup.find_all("li"):
                ...
                nation_alrban = (
                    report.find("a", class_="nlink")
                    .find("span", class_="nnameblock")
                    .text
                )
                if (" ejected and banned " in report.text) and (
                    nation_alrban not in banned_nations
                ):
